...
class GrpcProxyHandlerRegistry extends HandlerRegistry {
  ...
  private class RequestProxy extends ServerCall.Listener<byte[]> {
    ...
    @Override
    public void onReady() {
      logger.debug("Server call ready for " + methodDescriptor.getFullMethodName());
    }

    @Override
    public void onMessage(byte[] message) {
      logger.debug(
          "Received message for " + methodDescriptor.getFullMethodName() + ", forwarding...");

      if (clientCall == null) {
        // Create client call lazily on first message
        clientCall = proxyHandler.newCall(methodDescriptor, CallOptions.DEFAULT);

        clientCall.start(
            new ClientCall.Listener<>() {
              @Override
              public void onHeaders(Metadata headers) {
                logger.debug("Received headers from backend");
                serverCall.sendHeaders(headers);
              }

              @Override
              public void onMessage(byte[] message) {
                logger.debug("Received response from backend, forwarding to client");
                serverCall.sendMessage(message);
              }

              @Override
              public void onClose(Status status, Metadata trailers) {
                logger.debug("Backend call closed with status: " + status);
                serverCall.close(status, trailers);
              }

              @Override
              public void onReady() {
                logger.debug("Client call ready");
              }
            },
            metadata);
    ...
    @Override
    public void onHalfClose() {
      logger.debug("Half close for " + methodDescriptor.getFullMethodName());
      halfClosed = true;
      if (clientCall != null) {
    ...
    @Override
    public void onCancel() {
      logger.debug("Cancelling call for " + methodDescriptor.getFullMethodName());
      if (clientCall != null) {
    ...
    public void onComplete() {
      logger.debug("Call completed for " + methodDescriptor.getFullMethodName());
    }*/
  }
